...
class InferenceService(object):
    ...
    """消除旧文本，加入新文本"""
    def clean_expired_data(self, personal_data, similarity_message, message_time, duration):
        flag_data = personal_data[similarity_message]["windows"][duration]["timestamps"]
    ...
    """时间置换概念"""
    def time_management(self, personal_data, similarity_message, message_time):
        for key, value in personal_data[similarity_message]["windows"].items():
            start_time = value["start_time"]
            chat_duration = message_time - start_time
            if chat_duration < 0:
                return False
            elif chat_duration > key:
                self.clean_expired_data(personal_data, similarity_message, message_time, key)
        return True
    ...
    def process_request(self, request):
        ...
        if new:
            ...
            # 清除旧数据
            time_data = self.time_management(personal_data, similarity_message, message_time)
            if not time_data:
            ...
            for duration, threshold in scene_data.items():
                ...
                personal_data[similarity_message]['windows'][duration]["timestamps"].append(
                    message_time)
                no_similarity = len(
                    personal_data[similarity_message]['windows'][duration]["timestamps"])
                duration = str(int(duration / 60)) + "分钟"
                result["spammingTime"][duration] = {
            ...
            if len(personal_data[similarity_message]['windows'][duration]["timestamps"]) >= next(iter(g_config.get(-1).items()))[1]:
